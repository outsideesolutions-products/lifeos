...
async function bootstrap() {
  ...
  // bind to it; KNOWLEDGE_GRAPH_SERVICE_PORT remains the local
  // multi-service dev default (see .env.example) so `pnpm start:dev`
  // still works unchanged.
  const port = process.env.PORT ?? process.env.KNOWLEDGE_GRAPH_SERVICE_PORT ?? 4005;
