...
- The Milestone 0 code comments and READMEs that referenced B1–B5 as open (`authorization-input.interface.ts`, `authorization.service.ts`, `trust-authorization-service/README.md`, the root Prisma schema header, `@lifeos/db`'s package description) have been updated to reflect Round 7's resolution — documentation-only, no functional or type changes. **Milestone 0 is now fully complete.**
